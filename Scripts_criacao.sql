...
CREATE OR REPLACE VIEW VISAO_FUNCIONARIO AS
SELECT 
    F.COD_FUNCIONARIO,
    F.NOME,
    CASE 
        WHEN I.COD_INSPETOR IS NOT NULL THEN TRUE
        ELSE FALSE
    END AS IS_INSPETOR,
    F.EM_PLANTAO
FROM FUNCIONARIO F
LEFT JOIN INSPETOR I ON I.COD_FUNCIONARIO = F.COD_FUNCIONARIO;
...
CREATE OR REPLACE VIEW VISAO_MONITORAMENTO_EVENTOS AS
SELECT
...
FROM PREDIO P
JOIN MONITORAMENTO M ON P.ID_PREDIO = M.ID_PREDIO
JOIN FUNCIONARIO F ON M.COD_FUNCIONARIO = F.COD_FUNCIONARIO
JOIN REGISTRO_MONITORAMENTO RM ON RM.COD_FUNCIONARIO = M.COD_FUNCIONARIO
    AND RM.ID_PREDIO = P.ID_PREDIO
JOIN EVENTO E ON RM.ID_EVENTO = E.ID_EVENTO
JOIN CAMERA_EVENTO CE ON CE.ID_EVENTO = E.ID_EVENTO
JOIN CAMERA C ON C.ID_CAMERA = CE.ID_CAMERA
WHERE M.PLANTAO <= RM.PLANTAO; 
...
CREATE OR REPLACE VIEW VISAO_INSPETORES_E_MONITORAMENTO AS
SELECT
    I.COD_INSPETOR,
    ...
    E.DATA_DE_EVENTO AS DATA_EVENTO
FROM INSPETOR I
JOIN FUNCIONARIO F ON I.COD_FUNCIONARIO = F.COD_FUNCIONARIO
JOIN MONITORAMENTO M ON F.COD_FUNCIONARIO = M.COD_FUNCIONARIO
JOIN PREDIO P ON M.ID_PREDIO = P.ID_PREDIO
LEFT JOIN REGISTRO_MONITORAMENTO RM 
    ON RM.COD_FUNCIONARIO = I.COD_FUNCIONARIO 
    AND RM.ID_PREDIO = P.ID_PREDIO
LEFT JOIN EVENTO E ON RM.ID_EVENTO = E.ID_EVENTO;
...
DECLARE
    v_id_evento INT;
BEGIN
    -- Inserir novo evento e retornar o ID gerado
    ...
    VALUES (p_id_camera, v_id_evento);

    -- Associar o evento ao prédio
    INSERT INTO REGISTRO_MONITORAMENTO (COD_FUNCIONARIO, ID_PREDIO, PLANTAO, ID_EVENTO)
    SELECT 
        F.COD_FUNCIONARIO, 
        p_id_predio, 
        NOW(), 
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Erro ao registrar evento: %', SQLERRM;
END;
$$;
